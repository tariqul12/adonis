        ...
        <div class="top-bar mb-16">
            ...
            <ul class="nav nav-tabs" id="myProduct" role="tablist">
                ...
                <li class="nav-item" role="presentation" id="{{ str_replace(' ', '', $category->name) }}-tab">
                    <button class="nav-link" data-id="{{ $category->id }}" data-name="{{ str_replace(' ', '', $category->name) }}"
                        data-bs-toggle="tab" data-bs-target="#{{ str_replace(' ', '', $category->name) }}"
                        type="button" role="tab" aria-controls="{{ str_replace(' ', '', $category->name) }}" aria-selected="false">
                        {{ $category->name }}
                    </button>
                </li>
                @endforeach
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.length === 0) {
                        $(productsContainerId).html('<div class="text-center">No products found.</div>');
                    } else {
                        ...
                        `;
                            $(productsContainerId).append(productHTML); // পণ্য কনটেইনারে পণ্য অ্যাড করা
                        });
                    }
                },
                error: function() {
                    $(productsContainerId).html('<div class="text-center">Failed to load products.</div>');
                }
            });
        }
